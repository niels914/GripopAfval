...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-kpv-border bg-white/90 backdrop-blur">
      ...
      >
          ...
          {navigatie.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className={cn(
                  "inline-block whitespace-nowrap rounded-md px-2.5 py-2 text-sm font-medium transition-colors hover:bg-kpv-offwhite hover:text-kpv-paars",
        ...
        <div className="hidden xl:block">
          <Button asChild size="sm">
            <Link href="/afvalscan">Gratis afvalscan</Link>
          </Button>
        </div>

        <button
          type="button"
          className="rounded-md p-2 text-kpv-grijs xl:hidden"
          aria-expanded={open}
      ...
      {open && (
        <div id="mobiel-menu" className="border-t border-kpv-border bg-white xl:hidden">
